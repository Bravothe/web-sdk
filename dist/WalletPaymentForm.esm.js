...
function useWalletPaymentFlow(_ref) {
  ...
  var [processing, setProcessing] = useState(null); // 'quote' | 'charge' | 'ui' | null
  ...
  var {
    withMinProcessing,
    transitionTo
  } = useMemo(() => createTransitions(minProcessingMs, setProcessing, setView), [minProcessingMs]);

  // Build the boot function from the core (inject deps/setters)
  var boot = useMemo(() => createBoot({
    key,
    ent,
    amountValid,
    userWalletId,
...
function WalletPaymentForm(props) {
  ...
  var [forcedView, setForcedView] = useState(null); // 'success' | 'failed' | null
  var [forcedReason, setForcedReason] = useState('');
  var [forcedAmount, setForcedAmount] = useState(null);
  var [forcedCurrency, setForcedCurrency] = useState(null);
  var pendingSuccessPayloadRef = useRef(null); // call host after user closes success modal
  ...
  var [showBank, setShowBank] = useState(false);
